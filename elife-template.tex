...
For each force field combination, we used AM1-BCC charges computed using AMBER's \texttt{antechamber} on both the hosts and guests.

We wished to compare force fields directly, and as such, we used AM1-BCC partial atomic charges in each case.

The Lennard-Jones and 

\subsection{GAFF v1.7}
...
These calculations were performed with the Open Force Field Toolkit version 0.0.3 and SMIRNOFF99Frosst version 1.0.5.
...
\subsection{Selecitivity between orienations

\subsection{Selectivity between host molecule}
